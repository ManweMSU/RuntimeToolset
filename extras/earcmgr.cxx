...
public:
	...
	bool Archive(Console & console, bool silent)
	{
		...
		string file_name;
		try { pool = new ThreadPool; }
		catch (...) { if (!silent) console << TextColor(ConsoleColor::Red) << L"Failed to set up the thread pool." << TextColorDefault() << LineFeed(); return false; }
		try {
		...
		} catch (...) { if (!silent) console << TextColor(ConsoleColor::Red) << L"Failed to create a file." << TextColorDefault() << LineFeed(); return false; }
		try {
			uint flags = 0;
		...
		} catch (...) { if (!silent) console << TextColor(ConsoleColor::Red) << L"Failed to create a new archive." << TextColorDefault() << LineFeed(); return false; }
		if (!silent) {
		...
		for (int i = 0; i < Files.Length(); i++) {
			if (!silent) console << L"Packing file with name \"" << TextColor(ConsoleColor::Cyan) << Files[i].Name << TextColorDefault() <<
			...
			catch (...) { if (!silent) console << TextColor(ConsoleColor::Red) << L"Failed." << TextColorDefault() << LineFeed(); return false; }
			if (!silent) {
				console << TextColor(ConsoleColor::Green) << L"Succeed" << TextColorDefault() << LineFeed();
		...
		arc->Finalize();
		if (!silent) console << TextColor(ConsoleColor::Green) << L"Finished with " << (Time::GetCurrentTime() - started).ToShortString() << L"." << TextColorDefault() << LineFeed();
		return true;
	}
};
...
bool Execute(Array<Syntax::Token> & command, IO::Console & console, bool silent)
{
	if (command.Length() > 0) {
		if (command[0].Class == Syntax::TokenClass::CharCombo && command[0].Content == L"?") {
			...
			if (string::CompareIgnoreCase(command[0].Content, L"exit") == 0) {
				...
				if (!silent) console << L"Saving the archive list...";
				if (list.Save()) {
					if (!silent) console << TextColor(ConsoleColor::Green) << L"Succeed." << TextColorDefault() << LineFeed();
				} else { if (!silent) console << TextColor(ConsoleColor::Red) << L"Failed." << TextColorDefault() << LineFeed(); return false; }
			} else if (string::CompareIgnoreCase(command[0].Content, L"finish") == 0) {
				if (!silent) console << L"Saving the archive list...";
				if (list.Save()) {
					if (!silent) console << TextColor(ConsoleColor::Green) << L"Succeed." << TextColorDefault() << LineFeed();
					exiting = true;
				} else if (!silent) console << TextColor(ConsoleColor::Red) << L"Failed." << TextColorDefault() << LineFeed();
				return false;
				...
				if (!silent) console << L"Repositioned to \"" << TextColor(ConsoleColor::Yellow) << list.ListName << TextColorDefault() << L"\"." << LineFeed();
				if (!silent) console << L"Next \"save\" calls will use this name." << LineFeed();
				...
				} else if (!silent) console << L"(no elements)" << LineFeed();
				...
				selection = indicies;
				if (!silent) console << L"Selected " << TextColor(ConsoleColor::Cyan) << indicies.Length() << TextColorDefault() << L" files." << LineFeed();
				...
				selection.Clear();
				if (!silent) console << L"Excluded " << TextColor(ConsoleColor::Cyan) << count << TextColorDefault() << L" files." << LineFeed();
				...
				for (int i = 0; i < files->Length(); i++) {
					...
					list.Files << ent;
					if (!silent) console << L"Added \"" << TextColor(ConsoleColor::Cyan) << src << TextColorDefault() << L"\"." << LineFeed();
				}
				if (!silent) console << L"Added " << TextColor(ConsoleColor::Cyan) << count << TextColorDefault() << L" files." << LineFeed();
			} else if (string::CompareIgnoreCase(command[0].Content, L"create") == 0) {
				selection.Clear();
				ArchiveEntity ent;
				selection << list.Files.Length();
				list.Files << ent;
				if (!silent) console << L"New entry created." << LineFeed();
				...
				if (string::CompareIgnoreCase(command[1].Content, L"metadata") == 0) {
					...
					if (command.Length() > 3 && string::CompareIgnoreCase(command[3].Content, L"embed") == 0) {
						...
						} catch (...) { if (!silent) console << TextColor(ConsoleColor::Red) << L"Error setting embeded data." << TextColorDefault() << LineFeed(); return false; }
					} else {
						for (int i = 0; i < selection.Length(); i++) {
					...
					while (p < command.Length() && chain.Length() < 8) {
						...
						else { if (!silent) console << TextColor(ConsoleColor::Red) << L"Unknown method." << TextColorDefault() << LineFeed(); return false; }
						p++;
					...
					if (!silent) console << TextColor(ConsoleColor::Red) << L"Unknown property." << TextColorDefault() << LineFeed();
					return false;
				}
				...
				if (string::CompareIgnoreCase(command[1].Content, L"prepend") == 0 && command.Length() > 2) {
					...
					if (!silent) console << TextColor(ConsoleColor::Red) << L"Unknown operation." << TextColorDefault() << LineFeed();
					return false;
				}
			} else if (string::CompareIgnoreCase(command[0].Content, L"arc") == 0) {
				list.Archive(console, silent);
			} else return false;
		} else return false;
	} else return false;
	return true;
...
bool ParseCommandLine(Console & console)
{
	...
	for (int i = 1; i < args->Length(); i++) {
		...
		if (a[0] == L'-' || a[0] == L':') {
				...
				if (o == L'N') {
					...
					if (i < args->Length() - 1) {
						...
						try {
							...
							state.input.SetReference(new TextReader(input_stream));
							state.prompt = false;
							state.break_on_error = true;
						}
						catch (...) {
			...
			if (state.list_file.Length()) {
				console << TextColor(ConsoleColor::Red) << L"Archive list redefinition." << TextColorDefault() << LineFeed();
				return false;
			}
			state.list_file = args->ElementAt(i);
		}
	}
	return true;
}
...
int Main(void)
{
	SafePointer<Console> io = new Console;
	Console & console = *io;
	...
	if (!state.list_file.Length()) {
		if (!state.silent) {
			...
			console << L"  " << ENGINE_VI_APPSYSNAME << L" <list> :NSaci" << LineFeed();
			...
			console << L"  :a   - archives the list without command interpretation, then exits," << LineFeed();
			console << L"  :c   - create or erase existing list instead of loading," << LineFeed();
			console << L"  :i   - execute commands from source different of standard input," << LineFeed();
			console << L"           the file source of commands is given as the next argument," << LineFeed();
			console << L"           ASCII encoding if no signature present." << LineFeed();
		...
		try {
			if (state.create) {
				list.ListName = IO::ExpandPath(state.list_file);
				if (!state.silent) console << TextColor(ConsoleColor::Yellow) << L"Created an empty list." << TextColorDefault() << LineFeed();
			} else {
				if (!ArchiveList::Load(list, state.list_file)) {
					if (!state.silent) console << TextColor(ConsoleColor::Red) << L"Failed to open an archive list." << TextColorDefault() << LineFeed();
					return 1;
				}
				if (!state.silent) console << TextColor(ConsoleColor::Yellow) << L"Loaded a list with " << list.Files.Length() << L" files." << TextColorDefault() << LineFeed();
			}
			if (state.archive) {
				if (!state.silent) console << LineFeed();
				if (!list.Archive(console, state.silent)) return 1;
				return 0;
			}
			command_spelling.IsolatedChars << L'?';
			while (!state.input->EofReached()) {
				if (state.prompt) {
					console << LineFeed() << IO::Path::GetFileName(list.ListName).UpperCase() << TextColor(ConsoleColor::Magenta) << L"> " << TextColorDefault();
				}
				string command = state.input->ReadLine();
				...
				try {
					...
					if (!Execute(*decomposed, console, state.silent)) {
						if (exiting) break;
						else throw Exception();
					}
				}
				catch (Syntax::ParserSpellingException & e) {
					if (state.break_on_error) {
						if (!state.silent) console << TextColor(ConsoleColor::Red) << command + L": Invalid command syntax." << TextColorDefault() << LineFeed();
						break;
					}
					if (!state.silent) console << TextColor(ConsoleColor::Yellow) << L"Invalid command syntax. Type \"?\"." << TextColorDefault() << LineFeed();
				}
				catch (...) {
					if (state.break_on_error) {
						if (!state.silent) console << TextColor(ConsoleColor::Red) << command + L": Failed to execute the command." << TextColorDefault() << LineFeed();
						break;
					}
					if (!state.silent) console << TextColor(ConsoleColor::Yellow) << L"Failed to execute the command." << TextColorDefault() << LineFeed();
				}
			}
		}
		catch (...) {
